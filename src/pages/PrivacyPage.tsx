...
/**
 * Replaces plain email addresses in text with clickable mailto: links.
 * Keeps the surrounding text intact so content strings stay readable.
 */
function linkifyEmail(text: string) {
  ...
  return parts.map((part, i) =>
    emailRegex.test(part) ? (
      <a key={i} href={`mailto:${part}`} className="underline underline-offset-2 hover:opacity-80">{part}</a>
    ) : (
...
const sections = [
  ...
  { title: "8. Updates to This Policy", content: "This privacy policy may be updated periodically. Significant changes will be communicated through the platform and documented in our changelog. Last updated: February 2025." },
];
...
export default function PrivacyPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-16 lg:px-12">
      <SEO title="Privacy Policy" description="How OpenCHS handles, protects, and respects your data." />
